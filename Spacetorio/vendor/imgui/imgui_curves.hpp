...
namespace ImGui
{
    int Curve(const char *label, const ImVec2& size, int maxpoints, ImVec2
*points); float CurveValue(float p, int maxpoints, const ImVec2 *points); float
CurveValueSmooth(float p, int maxpoints, const ImVec2 *points);
};
*/
/*
    Example of use:
    ...
    int selectionIdx = -1;
    ...
    foo[0].x = ImGui::CurveTerminator; // init data so editor knows to take it
   from here
    ...
    ...
    ...
    float value_you_care_about = ImGui::CurveValue(0.7f, 10, foo); // calculate
...
static inline double ease(int easetype, double t) {
  ...
  case TYPE::ELASTICINOUT: {
    if (p < 0.5) {
      ...
      return 0.5 *
             (sin(-13 * pi2 * ((2 * p - 1) + 1)) * pow(2, -10 * (2 * p - 1)) +
              2);
    }
  }
...
#define tween$bounceout(p)                                                     \
  ((p) < 4 / 11.0   ? (121 * (p) * (p)) / 16.0                                 \
   ...
   : (p) < 9 / 10.0                                                            \
       ? (4356 / 361.0 * (p) * (p)) - (35442 / 1805.0 * (p)) + 16061 / 1805.0  \
...
static inline ImVec2 ImRemap(const ImVec2 &v, const ImVec2 &a, const ImVec2 &b,
  ...
  return ImVec2(ImRemap(v.x, a.x, b.x, c.x, d.x),
                ImRemap(v.y, a.y, b.y, c.y, d.y));
}

int Curve(const char *label, const ImVec2 &size, const int maxpoints,
          ImVec2 *points, int *selection, const ImVec2 &rangeMin,
  ...
  if (hovered) {
    ...
    ImVec2 pos =
        ImRemap(hoverPos, ImVec2(0, 0), ImVec2(1, 1), rangeMin, rangeMax);
    ...
    const ImVec2 hoverPosScreen =
        ImRemap(hoverPos, ImVec2(0, 0), ImVec2(1, 1), bb.Min, bb.Max);
    const ImVec2 p1s =
        ImRemap(points[left], rangeMin, rangeMax, bb.Min, bb.Max);
    const ImVec2 p2s =
        ImRemap(points[left + 1], rangeMin, rangeMax, bb.Min, bb.Max);
  ...
  if (draggingPoint) {
    ...
    pos = ImClamp(pos, ImVec2(pointXRangeMin, rangeMin.y),
                  ImVec2(pointXRangeMax, rangeMax.y));
  ...
  if (!IsMouseDragging(0) && GetActiveID() == id && selection &&
      *selection != -1 && currentSelection == -1) {
    ClearActiveID();
  }
  ...
  // bg grid
  drawList->AddLine(ImVec2(bb.Min.x, bb.Min.y + ht / 2),
                    ImVec2(bb.Max.x, bb.Min.y + ht / 2), gridColor1, 3);

  drawList->AddLine(ImVec2(bb.Min.x, bb.Min.y + ht / 4),
                    ImVec2(bb.Max.x, bb.Min.y + ht / 4), gridColor1);

  drawList->AddLine(ImVec2(bb.Min.x, bb.Min.y + ht / 4 * 3),
                    ImVec2(bb.Max.x, bb.Min.y + ht / 4 * 3), gridColor1);
  ...
  for (i = 0; i <= (smoothness - 1); ++i) {
    ...
    ImVec2 p =
        ImRemap(ImVec2(px, py), rangeMin, rangeMax, ImVec2(0, 0), ImVec2(1, 1));
    ImVec2 q =
        ImRemap(ImVec2(qx, qy), rangeMin, rangeMax, ImVec2(0, 0), ImVec2(1, 1));
    p.y = 1.0f - p.y;
    q.y = 1.0f - q.y;
  ...
  for (i = 1; i < pointCount; i++) {
    ImVec2 a =
        ImRemap(points[i - 1], rangeMin, rangeMax, ImVec2(0, 0), ImVec2(1, 1));
    ImVec2 b =
        ImRemap(points[i], rangeMin, rangeMax, ImVec2(0, 0), ImVec2(1, 1));
  ...
  if (hovered || draggingPoint) {
    ...
    for (i = 0; i < pointCount; i++) {
      ...
      if (hoveredPoint == i)
        ...
        drawList->AddCircle(p, pointRadiusInPixels,
  ...
  RenderTextClipped(ImVec2(bb.Min.x, bb.Min.y + style.FramePadding.y), bb.Max,
                    str, NULL, NULL, ImVec2(0.5f, 0.5f));
  ...
  if (ImGui::BeginPopupContextItem(label)) {
    ...
    if (ImGui::Selectable("Flip")) {
      for (i = 0; i < pointCount; ++i) {
        const float yVal =
            1.0f - ImRemap(points[i].y, rangeMin.y, rangeMax.y, 0, 1);
    ...
    if (ImGui::Selectable("Mirror")) {
      ...
      for (i = 0; i < pointCount; ++i) {
        const float xVal =
            1.0f - ImRemap(points[i].x, rangeMin.x, rangeMax.x, 0, 1);
      ...
      for (int row = 0; row < IM_ARRAYSIZE(items); ++row) {
        if (ImGui::MenuItem(items[row])) {
          for (i = 0; i < maxpoints; ++i) {
            ...
            points[i] = ImRemap(ImVec2(px, py), ImVec2(0, 0), ImVec2(1, 1),
                                rangeMin, rangeMax);
          }
        }
      }
